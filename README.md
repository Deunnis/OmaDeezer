...
- Album art is loaded from whatever `mpris:artUrl` the active MPRIS player reports - for the real Deezer app that's an `https://` URL on Deezer's own CDN (`*.dzcdn.net`), fetched directly by `Image.source`. Since any local process can claim the "deezer" MPRIS identity and control that URL, it's anchored to that CDN host specifically (not just the `https://` scheme - an arbitrary host would still let a rogue player point it at its own server), `file://` is allowed for a locally cached path, and the image is decoded with a bounded `sourceSize` so a malicious value can't force an oversized decode in the shared shell process.
